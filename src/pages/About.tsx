...
import { 
  Target, Rocket, Heart, Shield, Globe, Award, Sparkles, 
  Users, Zap, Star, Download, FileText, Cpu, Presentation 
} from 'lucide-react';
...
export default function About() {
  ...
  // Set default active tab based on query parameters, with 'philosophy' as fallback
  const [activeTab, setActiveTab] = useState<'philosophy' | 'proposal'>(
    tabParam === 'proposal' ? 'proposal' : 'philosophy'
  );

  // Scroll to the top when activeTab changes so the user starts from the beginning of each section
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    } else if (tabParam === 'philosophy') {
      setActiveTab('philosophy');
    }
  }, [tabParam]);

  const handleTabChange = (tab: 'philosophy' | 'proposal') => {
    setActiveTab(tab);
    setSearchParams({ tab });
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-28">
      ...
      {/* 🧭 Persistent Sub-Navigation Sticky Tabs */}
      <div className="sticky top-[112px] z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-center space-x-2 sm:space-x-8">
            <button
              onClick={() => handleTabChange('philosophy')}
              className={`py-5 px-3 sm:px-6 text-xs sm:text-sm font-black border-b-2 transition-all flex items-center gap-2 ${
            ...
            >
              <Cpu size={16} className={activeTab === 'proposal' ? 'text-brand' : 'text-slate-400'} />
              마케팅 백서 & AI 제안서
            </button>
          </div>
        </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'philosophy' ? (
              ...
              <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
                ...
                >
                  ...
                  <h1 className="text-4xl md:text-6xl font-black text-white mb-8 tracking-tighter leading-tight">
                    ...
                    <span className="text-brand text-2xl md:text-4xl block mt-4 font-black">전문직 50+를 위한 인생 2막 매칭 플랫폼</span>
                  </h1>
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: -15 }}
            transition={{ duration: 0.3 }}
            className="-mt-28" // Offsets the outer Proposal padding offset
          >
            <Proposal />
          </motion.div>
        )}
