...
export default function AiMatching() {
  ...
  const handleStartMatch = () => {
    ...
    // Step-by-step progress rotation
    const stepInterval = setInterval(() => {
      setMatchingStep((prev) => (prev < 3 ? prev + 1 : prev));
    }, 800);
              ...
              <div className="space-y-8">
                ...
                {isMatching && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="p-5 bg-indigo-50 border border-indigo-100 rounded-3xl space-y-2"
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-indigo-600 animate-ping shrink-0" />
                      <span className="text-[10px] font-black text-indigo-700 tracking-wider uppercase">실시간 정밀 매칭 분석 상태</span>
                    </div>
                    <p className="text-[11px] font-black text-slate-800 leading-snug animate-pulse min-h-[32px]">
                      {matchingProcessingSteps[matchingStep]}
                    </p>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden mt-1">
                      <motion.div 
                        className="bg-indigo-600 h-full"
                        initial={{ width: '0%' }}
                        animate={{ width: `${(matchingStep + 1) * 25}%` }}
                        transition={{ duration: 0.5 }}
                      />
                    </div>
                    <div className="flex justify-between items-center text-[9px] font-black text-slate-400 mt-1">
                      <span>AHP 다차원 역량 비교</span>
                      <span>{Math.round(((matchingStep + 1) / 4) * 100)}%</span>
                    </div>
                  </motion.div>
                )}
              </div>
